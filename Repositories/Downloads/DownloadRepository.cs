...
public class DownloadRepository: IDownloadRepository
{
    ...
    public async Task<IEnumerable<Download>?> GetDownloadsFilter(FilterDownloadDto filter)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            var pages = await response.Content.ReadFromJsonAsync<ApiResponse<IEnumerable<Download>>>();
            return pages!.Data;
        }
